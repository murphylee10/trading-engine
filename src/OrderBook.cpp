...
std::vector<Trade> OrderBook::addOrder(const Order& o) {
    ...
    if (o.side == Side::BUY) {
        // BUY side: we add resting parts to bids_, hit asks_
        auto priceCheck = [&](double lp, double lvl){ return lp >= lvl; };
        // match against asks_
        while (remaining > 0 && !asks_.empty()) {
            auto itLevel = asks_.begin();
            double lvlPrice = itLevel->first;
            if (!priceCheck(limitPrice, lvlPrice)) break;
            ...
            for (auto &t : newTrades) remaining -= t.quantity;
            if (queue.empty()) asks_.erase(itLevel);
        }
        // if it was a LIMIT and some remains, rest on bids_
        ...
        // SELL side: rest goes to asks_, hit bids_
        auto priceCheck = [&](double lp, double lvl){ return lp <= lvl; };
        while (remaining > 0 && !bids_.empty()) {
            ...
            trades.insert(trades.end(), newTrades.begin(), newTrades.end());
            for (auto &t : newTrades) remaining -= t.quantity;
            if (queue.empty()) bids_.erase(itLevel);
        }
        if (o.type == OrderType::LIMIT && remaining > 0) {
            Order rest = o;
            rest.quantity = remaining;
                                           ...
                                           Side                    incomingSide)
{
    ...
    while (remaining > 0 && !sideQueue.empty()) {
        ...
        Trade t;
        t.tradeId       = 0; // to be assigned by MatchingEngine
